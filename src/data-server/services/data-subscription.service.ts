...
import { OflowDataChannel, TICKER_THROTTLE } from '@/data-server/constants';
import { AppLogger } from '@/common/app-logger';
import { RtKline, RtPrice, RtTicker } from '@/data-service/models/realtime';
import {
  ChannelConsumer,
  DataChannelService,
...
interface RoomInfo {
  ...
  symbolKey?: string;
  channelConsumer: ChannelConsumer<RtPrice | RtTicker | RtKline>;
  channelTopic: string;
...
export class DataSubscriptionService {
  ...
  protected getRoom(req: SubscriptionRequest, symbolKey?: string) {
    ...
    let room = channel as string;
    if (exSymbols) {
      room = `${room}:e${exSymbols.length}:${symbolKey}`;
    } else {
      room = `${room}:${ex}:${symbol}`;
    }
    if (channel === OflowDataChannel.kline) {
  ...
  protected checkEmptyRooms(): any {
    for (const [_room, roomInfo] of this.roomInfos) {
      this.checkEmptyRoom(roomInfo).catch((err) => {
        console.error(err);
      });
    }
  ...
  handleDisconnect(_socket: Socket): any {
